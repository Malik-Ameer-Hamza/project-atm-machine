...
async function displayBanner() {
    await showBanner("ATM Machine", "You can perform similar functions like an atm machine in this program.");
}
...
async function login() {
    ...
    let spinner = createSpinner(chalk.rgb(79, 206, 93)("Authenticating")).start();
    await sleep();
...
async function customCashWithdraw() {
    let cashWithdrawn = await Amount();
    let spinner = createSpinner(chalk.rgb(79, 206, 93)(`\t Please Wait... \n  your transaction is in process`)).start();
    await sleep();
...
async function fixedCashWithdraw(cash) {
    let amount = 0;
    if (cash === "Cash RS. 500") {
        amount = 500;
    }
...
async function cashDeposit() {
    ...
    let spinner = createSpinner(chalk.rgb(79, 206, 93)(" Processing, Please Wait!")).start();
    await sleep();
